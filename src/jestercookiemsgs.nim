...
proc allMessages(data: CookieObj): seq[CookieMsg] =
  ...
  ## previous page as well as any messages generated by the current page.
  ##
  ## Each message has a ``j`` element for the ``judgement`` of the message,
  ## and a ``c`` element for the message itself.
  result = @[]
